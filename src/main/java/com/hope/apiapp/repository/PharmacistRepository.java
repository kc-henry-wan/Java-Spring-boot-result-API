...
public interface PharmacistRepository extends JpaRepository<Pharmacist, Long> {
	@Query("SELECT p.pharmacistId AS pharmacistId, p.firstName AS firstName, p.lastName AS lastName, p.mobile AS mobile, p.email AS email p.address1 AS address1, p.address2 AS address2, p.postalCode AS postalCode, p.status AS status FROM Pharmacist p  WHERE :status IS NULL OR p.status = :status ")
	...
	@Query("SELECT p.pharmacistId AS pharmacistId, p.firstName AS firstName, p.lastName AS lastName, p.mobile AS mobile, p.email AS email, p.address1 AS address1, p.address2 AS address2, p.postalCode AS postalCode, p.status AS status FROM Pharmacist p ")
	Page<PharmacistProjection> findAll(Pageable pageable, Specification<PharmacistProjection> spec);
